import { UserStageForeign, UserStageListParams } from "../../types";
export declare const UserStageForeignValidator: import("joi").ObjectSchema<UserStageForeign>;
export declare const UserStageListParamsValidator: import("joi").ObjectSchema<UserStageListParams>;
declare const UserStageValidator: {
    UserStageForeignValidator: import("joi").ObjectSchema<UserStageForeign>;
    UserStageListParamsValidator: import("joi").ObjectSchema<UserStageListParams>;
};
export default UserStageValidator;
//# sourceMappingURL=index.d.ts.map