...
var PhotoHuntService_exports = {};
__export(PhotoHuntService_exports, {
  default: () => PhotoHuntService_default,
  detail: () => detail4,
  details: () => details3,
  sync: () => sync,
  verify: () => verify5
});
module.exports = __toCommonJS(PhotoHuntService_exports);
...
var createMany = async (challenge, payload, session) => {
  ...
  const qrs = (await list3(qrParams)).list.map(({ id, code }) => ({
    id,
    code
  }));
...
var PhotoHuntService = { detail: detail4, details: details3, sync, verify: verify5 };
