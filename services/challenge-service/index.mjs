...
var list = async (params) => {
  ...
  const list2 = await challenge_model_default.find(filter).skip(skip).limit(limit).sort({ createdAt: -1 });
...
var create = async (payload) => {
  return db_default.transaction(async (session) => {
    ...
    const stageData = stageId ? await detail5(stageId, session) : null;
...
var update = async (id, payload) => {
  return await db_default.transaction(async (session) => {
    ...
    const item = await challenge_model_default.findOne({ _id: id, deletedAt: null });
    if (!item) throw new Error("challenge not found");
    const newStage = stageId ? await detail5(stageId) : null;
    const oldStage = item.stage?.id ? await detail5(item.stage.id) : null;
...
export { ChallengeDetails, _delete, create, challenge_service_default as default, detail2 as detail, list, update, updateContent, verify3 as verify };
