...
var QrService_exports = {};
__export(QrService_exports, {
  _delete: () => _delete3,
  default: () => QrService_default,
...
module.exports = __toCommonJS(QrService_exports);
...
var createMany2 = async (challenge, payload, session) => {
  ...
  const qrs = (await list4(qrParams)).list.map(({ id, code }) => ({
    id,
    code
  }));
...
var PhotoHuntService = { detail: detail4, details: details3, sync: sync2, verify: verify6 };
...
var setup5 = async (stageId, TID) => {
  const exist = await verify8(stageId, TID);
  if (exist) return exist;
...
var UserStageService = { list: list6, detail: detail6, setup: setup5, verify: verify8, submitState: submitState2 };
var UserStageService_default = UserStageService;

// _src/services/QrService/index.ts
var list4 = async (params) => {
  const skip = (params.page - 1) * params.limit;
  const filter = { deletedAt: null };
  if (params.status) filter.status = params.status;
