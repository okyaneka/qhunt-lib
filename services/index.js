...
var init = async (stage2, userStage, session) => {
  ...
  const payload = challenges.map((item) => {
    ...
    return {
      ...
      status: stage2.settings.unlockAll ? USER_CHALLENGE_STATUS.Discovered : USER_CHALLENGE_STATUS.Undiscovered,
      ...
      [
        {
          ...data,
          quest,
          slug: !count ? slug : slugify__default.default(`${slug}-${count}`),
...
exports.FeatureService = feature_service_default;
